...
from lenovotf import resourceUtil
import logging
...
def main(args):
    # 该策略仅支持单机多CPU/GPU分布式训练
    strategy = tf.contrib.distribute.MirroredStrategy()
    # 该策略支持多机多设备分布式训练
    # strategy = tf.contrib.distribute.CollectiveAllReduceStrategy()
    config = tf.estimator.RunConfig(train_distribute=strategy)
    # config = resourceUtil.ResourceUtil().update_config(config)
    logging.info(config.cluster_spec)
    logging.info("num_ps_replicas:" + str(config.num_ps_replicas))
    logging.info('master:' + config.master)
    logging.info(config.train_distribute)
    logging.info('task type: ' + config.task_type)
    logging.info(config.task_id)

    cluster = {'chief': ['localhost:2222'],
               'worker': ['localhost:2224', 'localhost:2225']}
    # config.cluster_spec = cluster
    # logging.info('after modify: '+config.cluster_spec)
    model = create_model()
